...
test -f ./metadata.json && grep -q '^  "name": "talend-cloudwatch",$' ./metadata.json
if [ $? -ne 0 ]; then
  fError "ERROR: You must be inside the directory puppet-cloudwatch to execute this script
  => cd ./puppet-cloudwatch"
fi
